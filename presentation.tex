...
\setbeamertemplate{frametitle}{%
  \vspace*{0.18cm}%
  \begin{beamercolorbox}[wd=\paperwidth,leftskip=0.8cm,rightskip=0.4cm]{frametitle}%
...
\begin{tcolorbox}[tcblue]
\small 累积 SupportCase → \mono{FailureMemory.analyze()} 批量归纳失败模式，\textbf{直接产出修复优先级}：
\end{tcolorbox}
...
\begin{tcolorbox}[tcblue]
\small FM 的输出是「该修哪里」；\\CM 的输出是「这题历史上有没有类似失败，给个 hint」。\\[0.2em]
两者互补，共同构成飞轮的学习闭环。
\end{tcolorbox}
\end{columns}
