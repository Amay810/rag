...
{\small\bfseries\color{Cmuted}与普通 RAG demo 的本质区别}\\[0.06cm]
\begin{tcolorbox}[enhanced, boxrule=0pt, colback=white, colframe=white, left=0pt, right=0pt, top=0pt, bottom=0pt]
{\footnotesize

% 第一列用 c 收缩宽度并自动居中
% 后面三列全部用 X，让它们自动平分剩余的总宽度，并保持垂直居中(m)和左对齐
...
\footnotesize 系统输出不是一段回答文本，而是一条可复盘的 \mono{SupportCase}：\\
...
{\small\bfseries\color{Cmuted}过渡页——快讲}\\[0.06cm]
{\footnotesize
...
{\small\bfseries\color{Cmuted}端到端流程}\\[0.08cm]
\begin{columns}[T]
...
\begin{tikzpicture}[
  ...
  nb/.style={b, draw=Cgreen!40, fill=Cgreelt,
             text=Cgreen!80!black, font=\fontsize{6.4}{7.4}\selectfont\bfseries, minimum width=2.18cm},
...
\node[nb, below=0.1cm of ret, xshift=-1.28cm] (pf) {+ 价格约束过滤};
\node[nb, below=0.1cm of ret, xshift=+1.28cm] (cd) {+ 复合查询分解};
\node[b,  below=0.62cm of ret]  (gen) {生成（LLM / catalog 模板）};
...
\footnotesize\begin{itemize}\setlength\itemsep{0.16em}
...
{\small\bfseries\color{Cmuted}为什么 dense alone 不够}\\[0.08cm]
\begin{columns}[T]
...
{\small\bfseries\color{Cmuted}三层可信度闸门}\\[0.08cm]
\begin{columns}[T]
\column{0.33\textwidth}
...
{\small\bfseries\color{Cmuted}科学方法论的基石}\\[0.08cm]
\begin{columns}[T]
\column{0.50\textwidth}
...
{\small\bfseries\color{Cmuted}评测驱动决策}\\[0.08cm]
{\footnotesize
...
{\small\bfseries\color{Cmuted}不靠直觉，靠历史失败案例}\\[0.08cm]
\begin{tcolorbox}[tcblue]
...
\begin{tabularx}{\textwidth}{l >{\hsize=1.1\hsize}X >{\hsize=0.9\hsize}X}
\toprule
...
\bottomrule
\multicolumn{3}{p{\textwidth}}{\scriptsize 其余 pattern 与 KB gap 放在备注页：\mono{zero\_retrieval\_handoff}、\mono{handoff\_cluster}、\mono{retrieval\_blind\_spot}、\mono{stale\_doc}}
\end{tabularx}}
...
{\small\bfseries\color{Cmuted}针对 embedding 的已测失效模式}\\[0.08cm]
\begin{columns}[T]
...
{\small\bfseries\color{Cmuted}最终量化证明（28 eval questions）}\\[0.08cm]
...
{\small\bfseries\color{Cmuted}失败驱动进化}\\[0.08cm]
\begin{columns}[T]
\column{0.50\textwidth}
...
{\small\bfseries\color{Cmuted}可信比「答得像」更重要}\\[0.08cm]
\begin{columns}[T]
...
%\vspace{0.2cm}
\begin{tcolorbox}[tcblue]
\small\textbf{关键说明}：freshness 的成功标准，不是 recall 提升，而是\textbf{避免过期价格或库存被当作确定事实输出}。\\[0.1em]
